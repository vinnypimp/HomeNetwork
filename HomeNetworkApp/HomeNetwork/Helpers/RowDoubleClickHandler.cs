...
namespace HomeNetwork.Helpers
{
    ...
    public sealed class RowSingleClickHandler : FrameworkElement
    {
        public RowSingleClickHandler(DataGrid dataGrid)
        {
            MouseButtonEventHandler handler = (sender, args) =>
            {
                var row = sender as DataGridRow;
        ...
        public static readonly DependencyProperty MethodNameProperty = DependencyProperty.RegisterAttached(
            "MethodName",
            typeof(string),
            typeof(RowSingleClickHandler),
            new PropertyMetadata((o, e) =>
            {
                var dataGrid = o as DataGrid;
                if (dataGrid != null)
                {
                    new RowSingleClickHandler(dataGrid);
                }
            }));
    }
}